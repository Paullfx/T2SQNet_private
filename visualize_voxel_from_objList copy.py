    ...
    for i, voxel_dict in enumerate(voxel_list):
        if isinstance(voxel_dict, dict):
            ...
            if isinstance(voxel_data, torch.Tensor):
                ...
                second_voxel_coords = second_voxel_coords[::10]
                ...
                o3d.visualization.draw_geometries([first_voxel_grid],
                                                  window_name=f'Voxel Hull {i + 1} - Part 1 - Experiment {exp_index}')
                # Visualize the second part in a separate window
                o3d.visualization.draw_geometries([second_voxel_grid],
                                                  window_name=f'Voxel Hull {i + 1} - Part 2 - Experiment {exp_index}')
            else:
